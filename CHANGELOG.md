...
- **annotation 索引优化** — `extract_rule_annotations()` 预构建 `HashMap` 索引，将规则查找从 O(n²) 降为 O(n)。64534 条规则场景下从 66 秒降至毫秒级
...
- **profile 条件支持数组** — `__when__.profile` 现在支持字符串或字符串数组（如 `profile: [config, subscribe]`），任一匹配即通过，与 platform 的数组语义一致。修复了数组形式被静默忽略导致 patch 无条件执行的安全隐患
