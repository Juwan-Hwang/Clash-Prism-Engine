...
const SANDBOX_HARDENING_JS: &str = r#"
                (function() {
                    ...
                    for (const _propName of _dangerous) {
                        try {
                            Object.defineProperty(globalThis, _propName, {
                                get: function() { throw new Error('Sandbox: ' + _propName + ' is permanently disabled'); },
                                set: function() { throw new Error('Sandbox: re-introducing ' + _propName + ' is permanently disabled'); },
                                configurable: false,
                                enumerable: false
                            });
                        } catch(e) {}
...
impl SandboxedRuntime {
    ...
    fn shutdown(self) -> u64 {
        ...
        self.timeout_handle.thread().unpark();
