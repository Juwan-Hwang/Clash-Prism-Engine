                    ...
                    if let Some(obj) = val.as_object() {
                        ...
                        if let Ok(cv) = config_val
                            && cv.is_object() {
                                ...
                                // 先存储到全局变量，再序列化
                                ctx.globals().set("__TMP_CONFIG__", cv.clone())
                                    .map_err(|e| format!("Failed to set temporary global for config serialization: {}", e))?;
                                let json_str: String = ctx.eval("JSON.stringify(globalThis.__TMP_CONFIG__)")
                                    .map_err(|e| format!("Failed to stringify returned config object: {}", e))?;
                                let _: std::result::Result<(), rquickjs::Error> =
                                    ctx.eval("delete globalThis.__TMP_CONFIG__");
                                if json_str.len() > MAX_MODIFIED_CONFIG_BYTES {
                                    return Err(format!(
                                        "Modified config size ({}) exceeds limit ({})",
                                        json_str.len(),
                                        MAX_MODIFIED_CONFIG_BYTES
                                    ));
                                }
                                let parsed = serde_json::from_str(&json_str)
                                    .map_err(|e| format!("Failed to parse returned config JSON: {}", e))?;
