...
const SANDBOX_HARDENING_JS: &str = r#"
                (function() {
                    ...
                    // 冻结 JSON 对象，防止脚本篡改 JSON.stringify
                    // （execute_with_write 依赖 JSON.stringify 提取配置）
                    if (globalThis.JSON) Object.freeze(globalThis.JSON);
                })()
            "#;
...
// 拒绝 null、非对象、数组、Promise、以及带有 toJSON 方法的对象（防止序列化为非对象）
if (__result === null || typeof __result !== 'object' || Array.isArray(__result)
    || typeof __result.then === 'function' || typeof __result.toJSON === 'function') {{
    var __type = Array.isArray(__result) ? 'array'
        : (__result && typeof __result.then === 'function') ? 'promise'
        : (__result && typeof __result.toJSON === 'function') ? 'object-with-toJSON'
        : typeof __result;
    log.error('main() must return a plain config object, got: ' + __type);
    return {{ success: false, error: 'main() must return a plain config object (not Promise, Date, or custom toJSON)' }};
}}
